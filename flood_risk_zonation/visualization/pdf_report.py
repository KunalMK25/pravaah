...
def _sih_executive_summary(sih_result, area_name: str, styles) -> list:
    """Build the PRAVAAH executive summary section."""
    story = []
    story.append(Paragraph("PRAVAAH — Executive Summary", styles["Title2"]))
    ...
    story.append(Paragraph(
        ...
        "All scores use declared, transparent weights — see PRAVAAH methodology section.</i>",
        styles["Caption"],
    ))
    return story
...
def _sih_habitation_table(sih_result, styles) -> list:
    ...
    story = []
    story.append(Paragraph("PRAVAAH — Vulnerable Habitation Assessment", styles["SectionHead"]))
    story.append(Paragraph(
...
def _sih_relocation_section(sih_result, styles) -> list:
    ...
    story = []
    story.append(Paragraph("PRAVAAH — Relocation Priority Detail", styles["SectionHead"]))
...
def export_pdf_report(result, output_path: str | Path, area_name: str = "Study Area", data_tier: int = 3,
    ...
    story.append(Paragraph("PRAVAAH — RISK ASSESSMENT REPORT", styles["Title2"]))
    story.append(Paragraph("Predictive Risk &amp; Vulnerability Assessment for At-Risk Habitations", styles["Subtitle"]))
    ...
    # ── PRAVAAH habitation intelligence sections (when sih_result is supplied) ─
    ...
    story.append(Paragraph(
        f"<i>This report was generated by PRAVAAH (Predictive Risk &amp; Vulnerability Assessment "
